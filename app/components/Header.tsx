    ...
    <>
      <div className="bg-ink text-[13px] text-[#edededf2] hidden">
        <div className="fix flex flex-wrap items-center justify-between gap-4 py-[9px] md:block hidden">
          ...
          <div className="flex items-center gap-2 opacity-90">
            ...
            24/7 emergency line · open now, Mon–Sun
          </div>
        </div>
      </div>
      ...
      <header className="sticky top-0 z-90 px-3 pt-3 sm:px-4 sm:pt-3.5">
        <div className="mx-auto max-w-[1200px]">
          <div
            className={`flex items-center justify-between gap-3 rounded-full border border-line bg-card pr-2.5 pl-5 shadow-[0_4px_16px_-10px_rgba(20,22,30,0.2)] transition-[padding,box-shadow] duration-300 sm:gap-6 sm:pr-3.5 sm:pl-6.5 ${
              scrolled
          ...
          {menuOpen && (
            <nav className="mt-2 flex flex-col gap-1 rounded-[26px] border border-line bg-card p-4 shadow-[0_16px_40px_-18px_rgba(20,22,30,0.28)] min-[920px]:hidden">
              {NAV_LINKS.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
